...
@Component({
    ...
    standalone: true,
    imports: [IonContent, IonTitle, IonToolbar, IonHeader, CommonModule, CapitalizePipe, RouterLink]
})
export class ListadoClientesRegistroComponent  implements OnInit {
  ...
  ngOnInit() {
    this.clienteService.traerTodosObservable().subscribe((clientes: Cliente[]) => {
      this.listaClientes = clientes.filter(cliente => cliente.estado === Estado.pendiente); // Aquí filtras por el estado que necesites
    });
  }
